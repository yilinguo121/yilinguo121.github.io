...
**本頁目錄**：[一支程式的骨架](#一支程式的骨架) ｜ [識別字（identifier）命名規則](#識別字identifier命名規則) ｜ [基本型別](#基本型別) ｜ [宣告與初始化](#宣告與初始化) ｜ [賦值相容性與隱式轉換](#賦值相容性與隱式轉換) ｜ [字面值與跳脫序列](#字面值與跳脫序列) ｜ [輸入輸出：`cin` 與 `cout`](#輸入輸出cin-與-cout) ｜ [算術運算與整數除法陷阱](#算術運算與整數除法陷阱) ｜ [遞增遞減與求值順序](#遞增遞減與求值順序) ｜ [常數](#常數) ｜ [註解與程式風格](#註解與程式風格) ｜ [本週練習題](#本週練習題)
