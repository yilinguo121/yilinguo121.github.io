...
$$
\text{now} = \sum_{i=0}^{n-1}\sum_{j=0}^{m-1} \mathbf{1}\big(a*{i,j}=b*{i,j}\big),
$$
...
* 每次旋轉 (f) 建構新矩陣需 (O(nm))（或 (O(mn))），每次相等性檢查需 (O(nm))。總共最多做 4 次，故時間複雜度為
...
$$
s_1=\sum_{\substack{0\le i\le11\(i+1)\text{ 為奇數}}}(x[i]-'0'),\qquad
s_2=\sum_{\substack{0\le i\le11\(i+1)\text{ 為偶數}}}(x[i]-'0').
$$
