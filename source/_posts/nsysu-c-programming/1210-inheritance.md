...
語法只是把父類別用逗號列出來。但多重繼承有個有名的麻煩：如果兩個父類別又各自繼承自同一個祖父類別，孫子就會拿到**兩份**祖父的資料（俗稱**菱形繼承問題**），要用 `virtual` 繼承才解得掉——那超出這學期範圍。

**實務上的建議**：多重繼承很少是最好的答案，多數情況用組合（把對方當成員變數）會更單純。
...
- `class Dog : public Animal` ＝ Dog 自動擁有 Animal 的 public 與 protected 成員。
- 子類別**碰不到父類別的 `private`**；想讓子孫用得到就宣告成 `protected`。
- 子類別建構子要用**初始化列表**呼叫父類別建構子；沒寫的話會去找父類別的預設建構子，找不到就編譯錯誤。
- **建構先父後子，解構先子後父**——這是筆試送分題。
- 覆寫（同名同簽名、發生在父子之間）跟重載（同名不同參數、發生在同一個作用域）是兩件事；子類別一旦定義同名函式，父類別的所有同名版本都會被遮蔽。
- 建構子、解構子、`operator=`、`friend` 關係**不會被繼承**。
- 先問「B **是一種** A 嗎」再決定用不用繼承；**多數情況組合比繼承好**。
