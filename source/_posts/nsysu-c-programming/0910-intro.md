...
一個沒教過的符號：`mkdir -p` 的 `-p` 表示**連中間不存在的上層資料夾一起建**，所以 `week01` 沒建過也沒關係。`&&`〈環境設置〉講過，所以 `make && ./Q1` 就是「編譯成功才執行」，失敗就會停在那裡不會亂跑（09/17 會教 C++ 裡的 `&&`，意思一樣是「兩個都成立才算成立」，只是寫在 `if` 的條件裡，不是寫在指令之間）。另外，指令後面 `#` 開頭的中文是寫給你看的說明，**不用跟著打**——這個 `#` 是 Terminal 的註解符號，跟 C++ 裡 `#include` 的 `#` 沒有關係。

**過關的樣子**：`make` 印出兩行 `g++ -std=c++11 -c Q1.cpp`、`g++ -std=c++11 -o Q1 Q1.o` 而且沒有任何紅字；`./Q1` 印出你的學號；`make clean` 會印出它執行的 `rm -f ...`（make 在回報它刪掉了什麼），接著 `ls` 的結果只剩 `Makefile` 和 `Q1.cpp`，`Q1` 和 `Q1.o` 都不見了。任何一步跟這不一樣，回〈環境設置〉對一次。
