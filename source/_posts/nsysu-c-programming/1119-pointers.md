...
| 課本題號 | 要用到的東西 | 先練 |
| --- | --- | --- |
| Ch9-2 | 逐字元轉換：字母往後移、數字加 5 取餘、空白刪掉、符號換成空白——`cctype`、`%` | 11/12 Q5、Q8 |
| Ch9-4 | 把一行裡四個字母的單字換成別的字，開頭大寫要保留——切單字、`substr` | 11/12 Q4 |
| Ch9-6 | 每行「曲名 - 作者」用 `find`／`substr` 拆開重排——字串搜尋＋讀檔 | 11/12 Q10、12/03 Q7 |
...
**實驗課題型加練**
以下照**去年（2025）第 9、10 週**實驗課的練習與歷年實驗課考卷的題型改寫（今年的投影片還沒出，題目可能會換）。去年指標這週實驗課出了一題**鏈結串列（linked list）**的堆疊與佇列——這個資料結構課本要到 Ch17 才教，但實驗課直接拿它來練 `new`／`delete` 和 `->`，所以這裡先用兩題把它講完；上機考的 `char**` 迷你資料庫和 `strtok` 關鍵字統計也是年年出現。
