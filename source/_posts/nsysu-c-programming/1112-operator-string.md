...
**實驗課題型加練**
Q6、Q7 的**題型**取自去年（2025）第 8、9 週實驗課的課堂練習（今年的投影片還沒出，題目可能會換）：先用一題「非成員函式＋`friend` 的比較運算子」暖身（10/29 留下來的題），再做運算子重載這週的招牌題「分數類別」——四則運算全部重載成成員函式、`<<` `>>` 用 `friend`，然後在 `main` 直接寫數學算式。題目不是我原創的：練的東西跟去年那幾題一樣，但題目本身、要算的式子、範例資料和解答都是我自己重寫的，不是原題。Q8、Q11 的題型取自歷年考古題（字母統計、羅馬數字；敘述與範例資料是重寫過的），Q9、Q10 是我自己出的補充題。
