...
| Ch3-13 | 隨機抽幾個不重複的號碼——`rand`＋重抽直到不重複 | Q17 |
...
**Q6. 費氏數列**
讀入 `n`（1 ≤ n ≤ 90：`long long` 到第 92 項都還裝得下，第 93 項起就會像 Q2 的 13! 一樣溢位，這裡保守取 90），輸出費氏數列前 `n` 項（$F_1 = 1, F_2 = 1$），以空白分隔。請用**迴圈**寫，並想想為什麼這題不該用單純遞迴。
...
**實驗課題型加練**
下面幾題照實驗課的題型出。三角形、高爾夫術語、3 的倍數是**今年第 2 週（09/24）**實驗課的三題練習題型；亂數大寫字母、亂數統計、奇偶位數和、猜拳是**去年第 2、3 週**的題型——助教提過第 3 週（10/01）會教函式、`rand` 可能也會出，但今年的題目還沒公布，這四題只是「照去年推測」；不管最後出不出，練了都不吃虧。這七題只給題目、範例輸出與思路，不放完整程式：實驗課的分數是當場檢查給的，助教也明講不能用 AI 生答案，請自己寫完再拿範例對。阿姆斯壯數、河內塔是上機考的經典題、抽不重複號碼對應課本勾選題，這三題附完整解答。
...
**Q11. 亂數大寫字母**（去年第 2 週的題型）
用 `rand()` 印出 10 個隨機的大寫英文字母（同一行，不用空格隔開），每次執行結果都要不一樣。

```text
某一次的輸出（每次不同）： LGSNYYPQLX
```

<details>
<summary><b>思路與驗算</b></summary>

- 大寫字母在 ASCII 表裡是 65（`'A'`）到 90（`'Z'`）連號，所以 `'A' + rand() % 26` 就是隨機一個大寫字母的碼；印之前要 `static_cast<char>(...)` 轉回字元，否則印出來是數字（09/17〈型別轉換〉那節的 `char` 與整數互轉）。
- 「每次執行都不同」靠 `srand(time(nullptr));`，整支程式在 `main` 開頭呼叫一次就好；放進迴圈裡反而會一直拿到同一個字母（正文講過）。
- 驗算：暫時把 `srand` 那行拿掉，連跑兩次會得到一模一樣的 10 個字母——反過來就證明了它的作用；也順便確認 10 個都是大寫。

</details>

**Q12. 亂數統計**（去年第 3 週的題型）
用 `rand()` 產生 100000 個 1–10000 之間的整數（不用存起來），統計這些數字**所有位數**裡 0、1、2 各出現幾次。例如亂數是 2047、15、810、3 時，0 出現 2 次、1 出現 2 次、2 出現 1 次。
...
- 外層 `for` 跑 100000 次，每次 `int n = rand() % 10000 + 1;`；內層 `while (n > 0)` 用 `n % 10` 取個位、`n /= 10` 去掉個位（Q3 的拆數字），取到的位數是 0、1、2 就把對應的計數器加 1。
- 三個計數器宣告時一定要歸零（09/17 雷區 ①），不然加在垃圾值上面。
- 驗算：先把 100000 暫時改成 4、把每個亂數印出來手算一次；再把範圍暫時改成 1–9，0 應該一次都不會出現。正式版本三個數字的次數加起來大約是「總位數 × 3/10」的量級，差太多就是拆位數寫錯。

</details>

**Q13. 奇數位與偶數位的和**（去年第 3 週的題型）
...
- 兩個和各寫成一個函式（例如 `int oddPositionSum(int n)`、`int evenPositionSum(int n)`），`main` 只負責讀、呼叫、印；`abs` 在 `<cstdlib>`。10/01 學了傳參考之後，可以改成一個函式一次帶回兩個和。
- 驗算：`263417` → A = 17、B = 6、|A − B| = 11；`5` → A = 5、B = 0；`1234` → A = 4 + 2 = 6、B = 3 + 1 = 4。
...
- 讀 `char`：`cin >> c;` 讀到 `'q'` 就 `break`；不是 `'0'`、`'2'`、`'5'` 就印 `invalid` 再讀下一個；`c - '0'` 把字元變成整數。
- 電腦出拳的函式：`rand() % 3` 得到 0、1、2，再對應成 0、2、5；`srand(time(nullptr))` 只在 `main` 開頭呼叫一次。
- 判勝負的函式：先處理平手，再把贏的三種組合直接列出來（0 贏 2、2 贏 5、5 贏 0），其餘就是輸；回傳 1／-1／0 讓 `main` 決定印什麼——函式只判斷、不印，比較好測。
- 驗算：電腦的拳是隨機的，測試時可以暫時讓出拳函式固定回傳 2，確認輸入 0 → 贏、2 → 平手、5 → 輸，再改回亂數。
