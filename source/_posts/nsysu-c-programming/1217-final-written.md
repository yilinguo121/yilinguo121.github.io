...
| 主題 | 一定要會 |
| --- | --- |
| 基本語法與分支（09/17） | 整數除法、型別轉換（→ 第 1 題）、`setprecision`、未初始化變數、`switch` 穿透（→ 第 2 題）、短路求值、`0 < x < 3` 的陷阱（→ 第 3 題） |
| 迴圈與函式（09/24） | 三種迴圈、`break`／`continue`、作用域遮蔽（→ 第 4 題） |
| 函式（10/01） | 傳值 vs 傳參考（→ 第 5 題）、重載規則（→ 第 6 題）、預設引數位置 |
