...
老師說過期中考**幾乎全部從課本勾選的習題出**（總覽頁那張表的「課本指定習題」欄，Ch1–Ch6），所以第一件事是把那些題目每一題自己寫過一遍（每篇文章開頭的〈這幾題各要用到什麼〉表列出每題需要的東西與對應練習）。接著下面每一項都要做到不看筆記能寫出來（括號是對應的模擬考題號）：
...
實驗課是另一門課，有自己的期末上機考（佔實驗課成績 40%），題型就是每週練習題那種。我看過的幾份歷年考古題有幾個共同特徵：連續輸入、讀到 `0` 或 `0 0` 結束、輸出格式要一模一樣、一題只有全對或零分。下面六題裡，Q1、Q3、Q4、Q5 的**題型**取自那些考古題（題目敘述、範例資料與解答是我重寫的，不是原題），Q2、Q6 是我照同樣風格自己出的；六題只用 Ch 1–6 的內容，期中前就可以練；主課的期中考**不是**這種題型（幾乎全從課本勾選題出），別搞混。六題建議 90 分鐘。
