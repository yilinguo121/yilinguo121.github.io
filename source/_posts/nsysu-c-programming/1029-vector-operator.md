...
- `vector` 是會自己長大的陣列：`push_back` 加、`size()` 問長度、`empty()` 判空。
- `v.size()` 的型別是 **`size_t`（無號）**，跟 `int` 比大小會被警告；空 vector 時 `v.size() - 1` 會變成天文數字。
- `v[i]` **不檢查範圍**，要加元素只能用 `push_back` 或先 `resize`；除錯時可改用 `v.at(i)`。
- vector 傳進函式：只讀用 `const vector<T>&`，要改用 `vector<T>&`，忘了 `&` 會整包複製。
- **運算子重載**＝定義 `+`、`==` 這些符號對自訂型別的意義，函式名字就是 `operator` 加那個符號。
- 不能發明新符號，也不能改變運算元個數與優先順序；意義要符合直覺。
