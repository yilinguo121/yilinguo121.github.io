...
```cpp
int* makeArray() {           // 想把陣列傳回去
    int a[5] = {1, 2, 3, 4, 5};
    return a;                // 危險：a 在函式結束的瞬間就消失了
}
```

編譯器通常會給 `warning: address of local variable 'a' returned`，執行結果則是垃圾值或當掉——因為區域陣列在函式結束時就被回收了，回傳的位址指向一塊已經不屬於你的記憶體。

這學期的標準作法是**由呼叫端準備好陣列，函式只負責填**：
...
- 索引從 **0** 到 **n-1**；`int a[5]` 沒有 `a[5]` 這一格。
- **越界不會有人擋你**，症狀可能是別的變數莫名其妙變了，或執行到一半 `Segmentation fault`。
- 陣列傳進函式時只傳了「第一格的位址」，所以**一定要額外傳長度**，而且函式改得到原陣列。
- 只讀不改的陣列參數寫成 `const int a[]`，讓編譯器幫你檢查。
- 部分填滿的陣列要**另外用一個變數記住有效長度**，迴圈條件用它而不是容量。
- 二維陣列傳進函式時，**第二維的大小必須寫死**。
- **不能回傳區域陣列**，標準作法是由呼叫端準備陣列、函式只負責填。
