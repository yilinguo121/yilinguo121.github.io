...
複製一個長字串很花時間，用 `&` 可以避免複製；再加上 `const`，等於對呼叫者宣告「**我保證不動你的東西**」。只要資料「比一個數字大」，幾乎一律用 `const 型別&` 傳，這是 C++ 的標準習慣（之後學到的陣列、`vector`、`struct` 也都適用）。
...
編譯器根據**你傳進去的引數型別與個數**決定要呼叫哪一個版本，這個挑選過程叫**重載解析（overload resolution，有些書譯作「多載」，是同一件事）**，順序大致是：
...
- **看到 `&` 就是會改到外面，沒有 `&` 就不會**——這是筆試最愛考的一句話。
- 只讀不改又比一個數字大的東西，一律用 `const 型別&` 傳：不複製、又保證不被改。
- **重載看的是參數列表**（型別或個數），**回傳型別不同不算重載**，會編譯錯誤。
- 預設引數只能放在參數列**最右邊**，而且宣告與定義只能寫一次。
- `assert(條件)` 用來檢查「這件事一定要成立」，交作業前確認它不會誤觸發。
- 與其整支寫完才編譯，不如**寫一個函式就用一個小 `main`（driver）測一個**。
