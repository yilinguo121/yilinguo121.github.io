...
| 課本題號 | 要用到的東西 | 先練 |
| --- | --- | --- |
| Ch4-3 | 選單購買：用**參考參數**在函式裡更新餘額，迴圈到買不起為止 | Q5、Q7、Q10 |
| Ch4-7／8／9 | 單位換算：輸入、計算、輸出各一個函式，`if-else` 決定換算方向，重複到使用者結束 | Q10 |
| Ch4-14 | 兩個時刻相差幾分鐘（要處理跨午夜）——`/`、`%` 拆時分、函式回傳 `int` | 09/17 Q6、Q2 |
...
在參數型別後面加一個 `&`，意思是「這個參數就是外面那個變數本人，不是複製品」。（注意這個 `&` 跟 09/17 的 `&&`「且」沒有關係；它還有第三個用法「取記憶體位址」，11/19 講指標才會遇到。）
...
```bash
g++ -std=c++11 -DNDEBUG -o app main.cpp
```
...
**實驗課題型加練**
以下三題的**題型**取自去年（2025）第 4 週實驗課的課堂練習（今年這週的投影片還沒出，題目可能會換）：同一個 swap 寫傳值版與傳參考版、預設引數、還有一題「反轉相加到迴文」的經典題。題目不是我原創的——練的東西跟去年那幾題一樣，但敘述、規則細節、範例資料和解答都是我自己重寫的，不是原題。Q9 的題型取自歷年考古題（連續輸入、讀到 `0 0` 結束的最大公因數與最小公倍數），Q10 配合課本勾選題。
...
**Q7. 反轉相加到迴文**
把一個數跟它的反轉相加，結果若不是迴文就再做一次，直到出現迴文。例如 87 → 87 + 78 = 165 → 165 + 561 = 726 → 726 + 627 = 1353 → 1353 + 3531 = 4884（迴文）。讀入起始數字，印出每一步、總共加了幾次、最後的迴文；超過 10 次還沒出現就放棄。要求兩個函式：`void reverseDigits(long long n, long long& result)` 用**傳參考**帶回反轉結果，`bool isPalindrome(long long n)` 判斷迴文。
...
**Q8. 字尾符號（預設引數）**
讀入一個不含空白的字串，用**同一個函式** `void decorate(const string& s, int len = 0)` 印兩次：第一次不給長度，一律在尾端加 `.`；第二次把字串長度傳進去，長度 ≥ 6 加 `!!!`、2–5 加 `!`、1 加 `?`。字串長度用 `s.size()` 取得（`string` 內建的功能，回傳字元數；10/29 會系統性介紹）。
...
預設引數 `= 0` 讓「沒給長度」變成一個可以判斷的狀態——真正的長度不可能是 0（`cin >> s` 讀到的字串至少有一個字元），所以 0 可以放心拿來當「沒給」的記號。`decorate(s, s.size())` 把 `size_t`（無號）傳給 `int` 參數會發生隱含轉換，g++ 在 `-Wall -Wextra` 下不會警告，而且字串長度不可能大到出問題；想寫得更明確可以用 `static_cast<int>(s.size())`。
