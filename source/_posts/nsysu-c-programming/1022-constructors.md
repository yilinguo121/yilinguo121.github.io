...
有兩種成員**不能在大括號裡用 `=` 指派**，只能靠初始化列表決定初值：
...
另外注意：`print()` 裡的 `fixed << setprecision(2)` 會**一直留在 `cout` 上（09/17 講過）**，所以最後一行的 `getBalance()` 也印成 `1200.00`。不想影響後面就補一句 `cout << defaultfloat << setprecision(6);`——`defaultfloat` 是 `fixed` 的反向開關，`6` 是 `cout` 原本的預設有效位數；只寫 `defaultfloat` 不夠，留著的 `setprecision(2)` 會把 `1200` 印成 `1.2e+03`。

> **補充：`inline`**。寫在 `class` 定義**裡面**的成員函式（像上面的 `deposit`、`withdraw`）自動算 `inline`——編譯器可以把函式內容直接展開在呼叫處。現在只要認得這個名詞，11/26 講 header 檔時會用到一次。
...
- 這個計數器有個盲點：`Widget d = a;` 這種**複製**不會經過 `Widget()`，`count` 不會加，但 `d` 消失時解構子照樣減，就會數錯。複製走的是「拷貝建構子」，Q4 會先補一個最簡單的版本，11/19 再完整講。

## 本週重點回顧

- 建構子**與類別同名、沒有回傳型別、建立物件時自動呼叫**；同名不同參數就是**重載**。
- `Circle a();` 是宣告函式，不是建立物件；無參數就寫 `Circle a;`。
- 初始化列表優先，`const` 與參考成員**不能在大括號裡指派**、只能用它；初始化順序依**宣告順序**（寫反會有 `-Wreorder` 警告）。
- 自己寫了任何建構子，預設建構子就沒了；要宣告陣列或被別人當成員時記得補一個（或用**建構子委派**共用邏輯）。
- 成員可以是別的類別的物件（**組合**）：成員先建構，才輪到外層建構子的大括號；解構順序相反。
- 解構子 `~類別名()` 在物件生命結束時自動呼叫。
- `static` **資料**成員全類別共用、這門課的寫法是在類別外定義一次；`static` 成員函式不必有物件就能呼叫，但只能碰 `static` 成員。
...
閏年規則：是 4 的倍數但不是 100 的倍數，或者是 400 的倍數。
...
**Q4. 物件計數器**
寫一個 `class Counter`，用 `static` 成員統計「目前存在幾個物件」與「總共建立過幾個物件」，並用一段程式驗證兩者的差別。用 `Counter f = c;` 複製出來的物件也要算進去。

```text
輸出：
inside : alive=6 created=6
outside: alive=2 created=6
```
...
public:
    ...
    Counter(const Counter&) { alive++; created++; }   // 拷貝建構子：用另一個物件複製出新物件時呼叫
    ~Counter() { alive--; }
...
離開內層區塊時 `c`、`d`、`e`、`f` 的解構子把 `alive` 減回 2，但 `created` 只加不減，所以還是 6。**拷貝建構子**那行是關鍵：`Counter f = c;` 不會呼叫 `Counter()`，而是呼叫「參數是 `const Counter&`」的那個建構子；編譯器預設會幫你生一個只複製成員的版本，它**不會**幫你 `alive++`，於是 `f` 消失時 `alive--` 就多扣了一次。自己寫一個、在裡面計數，數字才對。這是正文 `Widget` 那個例子的盲點，也是 11/19 三法則的第一次登場。
