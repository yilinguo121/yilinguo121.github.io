...
這支程式可以編譯、可以執行，但**畫面上不會有任何東西**——因為它什麼事都沒做。所有程式都是從這個骨架開始長出來的。
...
**字面值**（literal）就是直接寫在程式裡的固定值，像 `42`、`3.14`、`'A'`、`"Hello"`。編譯器會依照長相決定它的型別，也可以在後面加字母指定：
...
**原始字串（raw string，C++11）**：字串裡有一堆反斜線時（例如 Windows 路徑），逐個跳脫很煩。寫成 `R"(...)"`，括號中間的東西**原封不動**，不做任何跳脫：
...
- `endl` = 換行 + **強制清空緩衝區**；`'\n'` 只換行。（**緩衝區**是輸出的暫存區——程式不會每印一個字就送螢幕一次，而是先堆在暫存區、累積一批再送，這樣比較快。）大量輸出時用 `'\n'` 比較快。
...
- **運算子（operator）**：做運算的符號，例如 `+`、`-`、`*`、`/`、`%`。
- **運算元（operand）**：被運算的東西，`7 / 2` 裡的 `7` 和 `2` 就是運算元。
- **運算式（expression）**：運算子與運算元組成、**算得出一個值**的一串東西，例如 `7 / 2`、`a + b * 2`。

C++ 的算術運算子有五個：`+`（加）、`-`（減）、`*`（乘）、`/`（除）、`%`（取餘數）。乘除的優先順序比加減高，跟數學一樣，不確定就加括號。
...
>
> **未定義行為（undefined behavior，常簡稱 UB）**是很重要的名詞：代表 C++ 標準**沒有規定**這種寫法該產生什麼結果，所以編譯器想怎麼做都可以——可能看起來正常、可能當掉、也可能換台電腦就變另一個答案。之後講陣列越界、指標的時候還會一直遇到。
...
- 程式骨架永遠是 `#include` → `using namespace std;` → `int main()` → `return 0;`。
- **宣告變數時就給初值**。C++ 不會幫你歸零，累加器忘了寫 `= 0` 就是垃圾值。
- **整數除以整數還是整數**：`7 / 2` 得到 3。要浮點結果，至少一邊要是浮點（`7 / 2.0`）。算平均分數出錯九成是這個原因。
- 小數位數用 `fixed << setprecision(n)`（會一直生效）；欄寬用 `setw(n)`（**只影響下一個輸出**）。
- 固定不變的值用 `const` 取名字，不要在程式裡散落魔術數字。
- 三個之後天天會用到的名詞：**運算子**（`+ - * / %`）、**運算元**（被運算的值）、**運算式**（算得出一個值的一串東西）。
...
>
> **Q3 與 Q4 會用到「迴圈」與「陣列」**，這兩個是下一節（09/24）與 10/08 才正式教的東西。現在做不出來很正常——可以先跳過，或直接照著參考解答的結構寫一次，等學到再回來重做。
